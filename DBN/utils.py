...
def get_datasets(dataset_name: str, normalize=True, labels=False):
    ...
    if normalize:
        ...
        norm_val = (
            ...
            .cache()
            .shuffle(tf.data.experimental.cardinality(img_val))
            .batch(32)
            .prefetch(tf.data.AUTOTUNE)
        )
        norm_test = (
...
def get_pretrain_images(autoencoder, dataset_name: str, labels=False):
    ...
    r_img_train = (
        ...
        .batch(32)
        .map(
            lambda x, y: autoencoder(normalizer(x))
            if not labels
            else (autoencoder(normalizer(x)), y)
        )
        .prefetch(tf.data.AUTOTUNE)
    )
    r_img_val = (
